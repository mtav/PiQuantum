...
InputOutput::InputOutput(std::shared_ptr<SpiChannel> spi) 
    : Alarm(1000), spi(spi), chips(7), period(10), counter(0)
{
 ...
 * 
 */
std::shared_ptr<InputOutput> getInputOutput(void) 
{
...
void InputOutput::interrupt(void)
{   
    // Check display cycling variables
    // for all dc counters 
    for(int i = 0; i < (int)dc_counter.size(); i++)
    {
        if(dc_counter[i] < dc_max[i]) { dc_counter[i]++;}
        else 
        {
            dc_counter[i] = 0;
            dc_trigger[i] = true;
    ...
    for(Led * i : leds) 
    {
        ...
        for(int k = 0; k < 3; k++) 
        {
            ...
            { 
                // Write a 1 in correct position
                write[i -> get_positions()[k].chip] |= (1 << i -> get_positions()[k].line);
            }
        }
        // Apply the mask
...
int InputOutput::register_led(Led * led) 
{
    ...
    for(int k=0; k < 3; k++)
    {
        mask[led -> get_positions()[k].chip] |= (1 << led -> get_positions ()[k].line);
    }
